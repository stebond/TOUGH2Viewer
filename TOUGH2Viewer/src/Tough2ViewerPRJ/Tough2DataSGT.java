/*
 * $Id: Tough2DataSGT.java,v 1.8 2003/08/22 23:02:38 dwd Exp $
 *
...
/**
 * Create <code>SGTData</code> objects containing test data. These objects can
 * be used for graphics and analysis testing purposes.
 *
 * @author Donald Denbo
 * @version $Revision: 1.8 $, $Date: 2003/08/22 23:02:38 $
 * @since 1.0
 */
public class Tough2DataSGT {
    ...
    public final static int TOUGH2DATA = 4;
    //
    private int dataType_;
    private SGTData data_;
    private Collection coll_;
    private double[][] hist2D;
    private double[][] temp2D;
    double[] stat;
    int nx;
    int ny;
    int nz;
    ...
    public void createDataXY(int num1, int num2, float z) {
        stat = new double[2];
        hist2D = Tough2Viewer.dataobj.makeTough2Data3DXY(neighbors, num1, num2, stat, z);

    }

    public void createDataXZ(int num1, int num2, float y) {
        stat = new double[2];
        temp2D = Tough2Viewer.dataobj.makeTough2Data3DXZ(neighbors, num1, num2, stat, y);
        hist2D = new double[num2][num1];
        for (int i = 0; i < num2; i++) {
            for (int j = 0; j < num1; j++) {
                hist2D[num2 - i - 1][j] = temp2D[i][j];
            }
        }
        int debughere = 0;
    ...
    public void createDataYZ(int num1, int num2, float x) {
        stat = new double[2];
        temp2D = Tough2Viewer.dataobj.makeTough2Data3DYZ(neighbors, num1, num2, stat, x);
        hist2D = new double[num2][num1];
        for (int i = 0; i < num2; i++) {
            for (int j = 0; j < num1; j++) {
                hist2D[num2 - i - 1][j] = temp2D[i][j];
     ...
     *
     * @param dir direction of test data, Tough2DataSGT.X_SERIES,
     * Tough2DataSGT.Y_SERIES, Tough2DataSGT.PROFILE, Tough2DataSGT.LOG_LOG
     * @param range minimum and maximum overwhich to create
     * @param type type of series, Tough2DataSGT.SINE or Tough2DataSGT.RANDOM
     * @param xyz amplitude
     * @param off offset
     * @param per period
     * @see #PROFILE
     ...
     *
     */
    public Tough2DataSGT(int dir, Range2D range,
     ...
     *
     * @param dir direction of test data, Tough2DataSGT.XY_GRID,
     * Tough2DataSGT.XZ_GRID, Tough2DataSGT.YZ_GRID
     ...
     *
     */
    public Tough2DataSGT(int dir, Range2D range1, Range2D range2,
            int type, float amp, float off, float per, int nxx, int nyy, int nzz, float xyz, int n_neighbors) {
        ...
        values = getValues(axis1, num1, axis2, num2, type, dir, amp, off, per, xyz);
        ...
        switch (dir) {
            case XY_GRID:
                keyLabel.setText("XY grid");
                xMeta = new SGTMetaData("X", "m");
                yMeta = new SGTMetaData("Y", "m");
                sg = new SimpleGrid(values, axis1, axis2, "Test Series");
                break;
            case XZ_GRID:
                keyLabel.setText("XZ grid");
                xMeta = new SGTMetaData("X", "m");
                yMeta = new SGTMetaData("Z", "m");
                sg = new SimpleGrid(values, axis1, axis2, "Test Series");
                break;
            default:
            case YZ_GRID:
                keyLabel.setText("YZ grid");
                xMeta = new SGTMetaData("Y", "m");
                yMeta = new SGTMetaData("Z", "m");
                sg = new SimpleGrid(values, axis1, axis2, "Test Series");
        }
     ...
     *
     * @param dir direction of test data, Tough2DataSGT.TIME_SERIES
     * @param range minimum and maximum overwhich to create
     ...
     *
     */
    public Tough2DataSGT(int dir, TimeRange range, float delta,
            int type, float amp, float off, float per) {
        SimpleLine sl;
        SGTMetaData xMeta;
        SGTMetaData yMeta;
...
//      taxis[count].increment(delta*count, GeoDate.DAYS);
     ...
     *
     * @param dir direction of test data, Tough2DataSGT.ZT_SERIES
     ...
     */
    public Tough2DataSGT(Range2D xrnge, Range2D yrnge, int num) {
        SimplePoint sp;
    ...
    private double[] getValues(double[] axis, int num, int type,
        ...
        switch (type) {
            default:
            case Tough2DataSGT.RANDOM:
                for (count = 0; count < num; count++) {
                    values[count] = amp * Math.random() + off;
                }
                break;
            case Tough2DataSGT.SINE:
                for (count = 0; count < num; count++) {
    ...
    private double[] getValues(double[] axis1, int num1, double[] axis2, int num2,
            int type, int dir, float amp, float off, float per, float xyz) {

        //poi passa di qua//
        if (dir == XY_GRID) {
            createDataXY(num1, num2, xyz);
        }
        if (dir == XZ_GRID) {
            createDataXZ(num1, num2, xyz);
        }
        if (dir == YZ_GRID) {
            createDataYZ(num1, num2, xyz);
        }
        ...
        switch (type) {
            ...
            case Tough2DataSGT.RANDOM:
                for (count = 0; count < num1 * num2; count++) {
                    values[count] = xyz * Math.random() + off;
                }
                break;
            case Tough2DataSGT.SINE:
                count = 0;
                for (count1 = 0; count1 < num1; count1++) {
                    for (count2 = 0; count2 < num2; count2++) {
                        values[count] = xyz * Math.sin(axis1[count1] / per)
                                * Math.sin(axis2[count2] / per) + off;
                        count++;
                    }
                }
                break;
            case Tough2DataSGT.TOUGH2DATA:

                count = 0;
                for (count1 = 0; count1 < num1; count1++) {
                    for (count2 = 0; count2 < num2; count2++) {

                        values[count] = hist2D[count2][count1];
                        count++;

                    }
                }
                break;
            case Tough2DataSGT.SINE_RAMP:
                count = 0;
                ...
                for (count1 = 0; count1 < num1; count1++) {
                    for (count2 = 0; count2 < num2; count2++) {
                        values[count] = xyz * Math.sin(axis1[count1] / per)
                                * Math.sin(axis2[count2] / per) + off
                                + xyz * (ax1factr * count1 - ax2factr * count2);
                        count++;
                    }
                }
