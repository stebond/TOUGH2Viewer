 ...
 * <ol>
 * <li>A static convenience method which shows a modal font-chooser dialog and
 * returns the font selected by the user.
 * <li>A static convenience method for creating a font-chooser dialog where
 * <code>ActionListeners</code> can be specified to be invoked when the user
 * presses one of the dialog buttons.
 * <li>The ability to create instances of <code>JFontChooser</code> panes
 * directly (within any container). <code>PropertyChange</code> listeners can be
 * added to detect when the current "font" property changes.
 * </ol>
 * <p>
 *
 * @author Adrian BER
...
public class JFontChooser extends JComponent {
    ...
    /**
     * Listener used to update the font in the components. This should be
     * registered with the selection model.
     */
    private LabelUpdater labelUpdater = new LabelUpdater();
    /**
     * True if the components are being updated and no event should be
     * generated.
     */
    private boolean updatingComponents = false;

    /**
     * Listener class used to update the font in the components. This should be
     * registered with the selection model.
     */
     ...
     * pressed, the color-chooser's color will be reset to the font which was
     * set the last time <code>show</code> was invoked on the dialog and the
...
//        return new Font(map);
    ...
    /**
     * Updates the font in the preview component according to the selected
     * values.
     */
    ...
    /**
     * Sets the current font of the font chooser to the specified font. The
     ...
     *
     * @param previewText the preview text, if null the font name will be
     * displayed
     */
...
class DefaultFontSelectionModel implements FontSelectionModel {
    ...
    /**
     * Creates a <code>DefaultFontSelectionModel</code> with the current font
     * set to <code>Dialog, 12</code>. This is the default constructor.
     */
    ...
    /**
     * Creates a <code>DefaultFontSelectionModel</code> with the current font
     * set to <code>font</code>, which should be non-<code>null</code>. Note
     * that setting the font to <code>null</code> is undefined and may have
...
interface FontSelectionModel {
    ...
    /**
     * Sets the selected font to <code>font</code>. Note that setting the font
     * to <code>null</code> is undefined and may have unpredictable results.
     * This method fires a state changed event if it sets the current font to a
     * new non-<code>null</code> font.
     *
